...
load_driver() ->
    ...
    erl_ddll:load_driver(Dir, "RE_drv").
...
%% to the port
compile(Pat, Options) when binary(Pat), list(Options) ->
    Flags = copt_to_flag(Options,0),
    P = open_port({spawn, 'RE_drv'}, [binary]), 
...
match(Port, Str, Opts) when binary(Str) ->
...
cmatch(Pat, Str, CompOpts) when binary(Str), binary(Pat)  ->
